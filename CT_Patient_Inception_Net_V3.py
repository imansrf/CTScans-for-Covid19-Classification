...
def Train_fold(i,X_train,y_train,X_val,y_val):
    ...
    history = model.fit(X_train, y_train, epochs=500, batch_size=64, validation_data=(X_val, y_val),verbose=1,callbacks=[RocAuc,checkpoint],class_weight = 'balanced')
    draw_history(history,f'CNN_2c_fold_{i+1}_training_track')


def Inception_Net_V3():
    base_model = keras.applications.inception_v3.InceptionV3(include_top=False, weights=None,input_shape=(200, 200, 10))
    ...
    model.compile(optimizer=optimizers.Adam(lr=0.0001,decay=0.05), loss='categorical_crossentropy', metrics=['accuracy'])
    return model
